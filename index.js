//Dependancies
  ...
  const argv = require("minimist")(process.argv.slice(2), {alias:{s:"size", o:"overlap"}, default:{size:256, overlap:0}})
  const progress = require("cli-progress")
  
//Generate images
  if (fs.existsSync("raw")) {
    //Init
      let { size, overlap } = argv
        ...
        //Generate sub-images
          for (let x = 0; x + size < width; x = Math.floor(x + size * (1 - overlap)))
            for (let y = 0; y + size < height; y = Math.floor(y + size * (1 - overlap))) 
              image.extract({left:x, top:y, width:size, height:size}).toFile(`computed/${file.replace(/\.(.+?$)/, (m, a) => `-${uuid()}.${a}`)}`)
          bar.increment()
      })
  }